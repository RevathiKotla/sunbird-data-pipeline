...
class ContentUpdaterStreamTaskTest extends BaseTestSpec {
  ...
  val extConfig: ContentCacheUpdaterConfig = new ContentCacheUpdaterConfig(config);
...
*/
 ...
 /* "Extraction job pipeline" should "extract events" in {
    ...
    val task = new ContentCacheUpdaterStreamTask(extConfig, mockKafkaUtil);
    task.process()
  ...
  "ContentUpdaterTask" should "test the dialcode function" in {
  ...
  "ContentUpdaterTask" should "test the content function" in {
    ...
    when(mockKafkaUtil.kafkaEventSource[Event](extConfig.inputTopic)).thenReturn(new ContentDialCodeSource)
    val task = new ContentCacheUpdaterStreamTask(extConfig, mockKafkaUtil);
    task.process()
...
class InputSource extends SourceFunction[Event] {
  ...
  override def run(ctx: SourceContext[Event]) {
    ...
    val gson = new Gson()
    val event1 = gson.fromJson(EventFixture.dialcodedata1, new util.LinkedHashMap[String, AnyRef]().getClass)
...
class ContentDialCodeSource extends SourceFunction[Event] {
  ...
  override def run(ctx: SourceContext[Event]) {
    val gson = new Gson()
    val event1 = gson.fromJson(EventFixture.contentData1, new util.LinkedHashMap[String, AnyRef]().getClass)
    ctx.collect(new Event(event1, 0))
